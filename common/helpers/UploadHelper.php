...
class UploadHelper{
    ...
    public static function __upload($image, $options = []){
        ...
        $f = UploadHelper::factory(\Yii::getAlias('@frontend').'/web/'.$options['directory'], false);
        $f->file($image);

        if(array_key_exists('filename', $options)){
            $f->set_filename($options['filename']);
        }
        ...
        if(array_key_exists('directory', $options)){
            $directory = DIRECTORY_SEPARATOR.$options['directory'];
        }

        $f['filename'] = $directory.DIRECTORY_SEPARATOR.$f['filename'];
    ...
    public function set_filename($filename) {
        preg_match('/\.[0-9a-zA-Z]{1,5}$/', $this->file['original_filename'], $matches);
        $this->filename = strtolower($filename.$matches['0']);
    }
    ...
    protected function create_destination() {
        \Yii::trace($this->root . $this->destination);
